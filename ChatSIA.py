...
class ChatSIA():
    ...
    def handle_message(update, context):
        ...
        respons = R.chatSIAResponse(text)
        if respons == True:
    ...
    def main(self):
        ...
        conv_handler = ConversationHandler(
            entry_points=[MessageHandler(Filters.text, ChatSIA.handle_message)],
            states={
                PENILAIAN: [MessageHandler(Filters.regex('^(Sesuai|Tidak Sesuai)$'), ChatSIA.penilaian)],
            },
            fallbacks=[CommandHandler('cancel', ChatSIA.cancel)],
        )
        dp.add_handler(CommandHandler("start", ChatSIA.start_command))
        dp.add_handler(CommandHandler("help", ChatSIA.help_command))
